function [D,Gamma,err,gerr, varargout] = ksvd_JointTrain(params,varargin)
% Data Model:  
% 
...
% 
% 
% Goal: The goal is to learn [Psi_cx; Psi_cy] among these dictionaries;
% 
% 
% References
% ------------
...
% Global Conf. Signal Inform. Process. IEEE, 2016, pp. 162–166.
...
info_all = sprintf('Joint Dictionary Learning ... \n');
...
for iter = 1:iternum
  ...
  if (testgen)
	...
	% compute the residual in terms of RMSE for X and Y
	Fnorm.X = sum(sum((data(ind1_row,:) - D(ind1_row,:)*Gamma).^2));
	Fnorm.Y = sum(sum((data(ind2_row,:) - D(ind2_row,:)*Gamma).^2));
	Fnorm.XY = Fnorm.X + Fnorm.Y ;
	RMSE.XY(iter, 1) = sqrt( Fnorm.XY / numel(data)) ;
	RMSE.X(iter, 1) = sqrt( Fnorm.X / numel(data(ind1_row,:)));
	RMSE.Y(iter, 1) = sqrt( Fnorm.Y / numel(data(ind2_row,:))); 
	
% 	% separate two dictionaries from the whole dictionary and normalize their atoms.
% 	DX = D(ind1_row,:);
% 	DY = D(ind2_row,:);
% 	DX = 0.9.*DX./repmat(sqrt(sum(DX.^2)),[size(DX,1),1]);
% 	DY = 1.1.*DY./repmat(sqrt(sum(DY.^2)),[size(DY,1),1]);
% 	D = [DX;DY];
...
% 	DX = D(ind1_row,:);
% 	DY = D(ind2_row,:);
% 	DX = DX./repmat(sqrt(sum(DX.^2)),[size(DX,1),1]);
% 	DY = DY./repmat(sqrt(sum(DY.^2)),[size(DY,1),1]);
% 	D = [DX;DY];
% 	D = D./repmat(sqrt(sum(D.^2)),[size(D,1),1]);
...
info_all = sprintf(['%s\n', '==================================================',...
				'\nJoint Dictionary Learning completed !! \n'],info_all);
