...
% Coupled dictionary learning algorithm adapted from K-SVD to solve following problem.
...
% 
% min_{Psi_cx, Psi_cy, Psi_x, Psi_y, Z_c, Z_x, Z_y} || [X ; Y] - [Psi_cx; Psi_cy] * Z_c - [Psi_x, 0; 0, Psi_y] * [Z_x; Z_y] ||_F^2;
% s.t. ||z_ci||_0 + ||z_xi||_0 + ||z_yi||_0 <= s; \forall i
% We adapted K-SVD to alternatively train common dicts and unique dicts. 
% During the training, all the sparse representations are updated.
% In the first stage, we fix the unique dicts, i.e. Psi and Phi, 
% and then update the update common dicts, i.e., Psi_c and Phi_c 
% and all the sparse representations, i.e., Z U, V. Repeat several iterations in this stage. 
% Then we move to the next stage, where we fix the common dicts, i.e., Psi_c and Phi_c, 
% and then update the unique dicts, i.e. Psi and Phi and all the sparse representations, i.e., Z U, V. 
% Repeat several iterations in this stage. Then return to the first stage.
