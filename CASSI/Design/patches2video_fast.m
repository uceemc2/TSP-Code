...
function im = patches2video_fast(M_patches, N1, N2,N3, delta1, delta2)
...
% N3 si the channel number

if ( N1 > 65535 ) || ( N2 > 65535 ) 
    error('The image size should be smaller than 65535 x 65535');
end

[patch_size1, num_patches] = size(M_patches);
patch_size = patch_size1/N3;
n1 = sqrt(patch_size);
n2 = n1;
...
for k = 1 : num_patches
    ...
    im(coor_x, coor_y,:) = im(coor_x, coor_y,:) + reshape(M_patches(:, k), [n1, n2,N3]);
    M_weight(coor_x, coor_y) = M_weight(coor_x, coor_y) + 1;
end

im = bsxfun(@rdivide,im, M_weight);
